...
class PlayState extends MusicBeatState
{
	...
	override public function update(elapsed:Float)
	{
		...
		var mult:Float = FlxMath.lerp(1, iconP1.scale.x, CoolUtil.boundTo(1 - (elapsed * 19 * playbackRate), 0, 1));
		iconP1.scale.set(mult, mult);
		iconP1.updateHitbox();

		var mult:Float = FlxMath.lerp(1, iconP2.scale.x, CoolUtil.boundTo(1 - (elapsed * 19 * playbackRate), 0, 1));
	...
	public function RecalculateRating(badHit:Bool = false) {
		...
		if(ret != FunkinLua.Function_Stop)
		{
			...
			// Rating FC
			if (sicks > 0) ratingFC = ratingFCS[0];
			if (goods > 0) ratingFC = ratingFCS[1];
			if (bads > 0 || shits > 0) ratingFC = ratingFCS[2];
			if (songMisses > 0 && songMisses < 10) ratingFC = ratingFCS[3];
			else if (songMisses >= 10) ratingFC = ratingFCS[4];
		}
